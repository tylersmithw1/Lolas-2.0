...
def test_product_matching_order_preserved():
    ...
    data = [
        {
            "product": "Tropicana Trop50 No Pulp Calcium + Vitamin D Orange Juice, 89 Oz Bottle",
            "price": 5.18,
        },
        {
            "product": "Goodbelly Probiotics No Sugar Added Peach Mango Orange Juice Drink, 1 Quart",
            "price": 2.98,
        },
        {
            "product": "Minute Maid, Premium Strawberry Lemonade, 59 Fl. Oz.",
            "price": 1.50,
        },
...
def test_get_recommendations_success(mock_rec_service):
    ...
    response = client.post(
        "/recommendations",
        json={"product_name": "Some juice product", "column_name": "sugar"},
    )
...
def test_get_recommendations_no_match(mock_rec_service):
    ...
    response = client.post(
        "/recommendations",
        json={"product_name": "Nonexistent Product", "column_name": "sugar"},
    )
...
def test_get_recommendations_invalid_column(mock_rec_service):
    ...
    response = client.post(
        "/recommendations",
        json={"product_name": "A - Pepperoni Pizza", "column_name": "invalid_column"},
    )
...
def test_get_recommendations_service_fail():
    ...
    app.dependency_overrides[RecommendationService] = (
        lambda: MockReccomendationFailingService()
    )

    response = client.post(
        "/recommendations",
        json={"product_name": "A - Pepperoni Pizza", "column_name": "sugar"},
    )
...
def test_get_ai_recommendations_ranking_success(mock_ai_rec_service):
    ...
    response = client.post(
        "/ai-recommendations",
        json={"full_product_name": "Tai Pei Pork Egg Rolls, 8 Count, 24.5 Oz"},
    )
...
def test_ai_recommendation_exception():
    ...
    # send data with an empty search string to trigger an exception
    response = client.post(
        "/ai-recommendations", json={"full product name": ""}
    )  # doesnt match full_product_name
...
def test_ai_recommendation_chat_fail():
    ...
    app.dependency_overrides[RecommendationService] = (
        lambda: MockAiRecResponseFailingService()
    )

    response = client.post(
        "/ai-recommendations", json={"full_product_name": "shrimp rolls"}
    )
