...
def test_get_chat_response(mock_get_chat, mock_create_agent, chat_service):
        ...
        "messages": [
            ...
            AIMessage(
                content=[
                    {
                        "type": "text",
                        "text": "<thinking> To rank the healthfulness of pepperoni pizza...",
                    },
                    {
                        "type": "tool_use",
                        "name": "initial_data_search",
                        "input": {"query": "pepperoni pizza"},
                    },
                ]
            ),
            AIMessage(
                content="<thinking> Based on the retrieved data, I will rank the pepperoni pizza products...</thinking>\n\n<json> {'ranking': ['Real Good Pepperoni Pizza Snack Bites', 'Pepperoni Pizza Snack Rolls'] } </json>"
            ),
        ]
    }
    ...
    assert result == {
        "ranking": [
            "Real Good Pepperoni Pizza Snack Bites",
...
def test_initial_data_search_exact_match(search_mock_df):
    ...
    result = initial_data_search.invoke(
        {"query": query, "df": search_mock_df, "threshold": 95}
    )
...
def test_initial_data_search_no_match(search_mock_df):
    ...
    query = "Takis"
    result = initial_data_search.invoke(
        {"query": query, "df": search_mock_df, "threshold": 90}
    )
...
def test_intiial_data_search_multiple_matches(search_mock_df):
    ...
    query = "Water"
    result = initial_data_search.invoke(
        {"query": query, "df": search_mock_df, "threshold": 80}
    )
    data = json.loads(result)
...
def test_get_closest_product_match_not_found(recommender_service, recs_mock_df):
    ...
    with patch("services.recommendation_service.process.extractOne") as mock_extract:
        mock_extract.return_value = (
            ...
            40,
            0,
        )  # returns value below threshold, so should return None
...
def test_recommendations_by_column_valid(
    mock_cosine, mock_vectorizer, mock_extract, recommender_service, recs_mock_df
):
    """test for valid recommendations"""
    mock_extract.return_value = ("A - Pepperoni Pizza", 95, 0)

    mock_vec_instance = MagicMock()
    mock_vec_instance.transform.return_value = np.array(
        [[1.0], [0.9], [0.8], [0.7], [0.6], [0.5]]
    )
    mock_vectorizer.return_value = mock_vec_instance

    mock_cosine.return_value = np.array(
        [[0.9, 0.85, 0.8, 0.02, 0.7]]
    )  # assign similarity scores to the products. 'meat lovers' given a low similarity score here

    with patch("services.recommendation_service.REC_DF", recs_mock_df):
        result = recommender_service.recomendations_by_column(
            "A - Pepperoni Pizza", "energykcal per 100"
        )
    ...
    assert (
        "A - Pepperoni Pizza" not in result["ranking"]
    )  # make sure the original product is excluded
    assert (
        len(result["ranking"]) == 3
    )  # 3 items are returned cause realistically all aside from 'meat lovers' should be similar (the word 'pizza' is in all of them)
    assert (
        "D - Meat Lovers" not in result["ranking"]
    )  # Ensure non-pizza products are excluded
    assert (
        "B - Veggie Pizza" in result["ranking"]
    )  # Ensure pizza-related products are included
    assert (
        "C - Cheese Pizza" in result["ranking"]
    )  # Ensure pizza-related products are included
    assert (
        "E - Supreme Pizza" in result["ranking"]
    )  # Ensure pizza-related products are included
...
def test_recommendations_by_column_no_match(mock_extract, recommender_service):
    ...
    with patch("services.recommendation_service.REC_DF", mock_df):
        result = recommender_service.recomendations_by_column(
            "Unknown Product", "energykcal per 100"
        )
        assert result is None
...
def test_ai_recommendations_response(
    mock_extract_json, mock_get_chat, mock_create_agent, recommender_service
):
    ...
    result = recommender_service.getRecommendationResponse(
        "Real Good Pepperoni Pizza Snack Bites, 8.5 Oz Box, 8 Count"
    )
