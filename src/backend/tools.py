...
@tool
def initial_data_search(query, df=df, threshold=80, output_filename="filtered_results.xlsx"):
    """Use this tool to retrieve the immediate food data relating to the user's search term."""

    for column in df.select_dtypes(include=['object']).columns:
        df[column] = df[column].fillna('').astype(str)
    
    df.drop(["price", "servingspercontainer", "servingsize", "energykcal", "fat", "saturatedfat", "transfat", "carbohydrates", "sugar", "salt", "fibre", "protein", "ingredients", "image", "redmeat", "shelfrank", "packsize", "packunit", "energykcal per 100", "fat per 100", "saturatedfat per 100", "transfat per 100", "carbohydrates per 100", "sugar per 100", "salt per 100", "fibre per 100", "protein per 100"], axis=1, inplace=True)
